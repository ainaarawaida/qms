    ...
    <tr>
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM ncr WHERE jabatan_unit = '$jabatan_unit' AND  status = 'done' LIMIT $start_from, $per_page");
        $stmt->execute();
      ...
      <tr>
        ...
        <td>
          <a href="listNcrDihantar.php?no_id=<?php echo $readrow['no_id']; ?>" class="btn btn-success btn-xs" role="button">Lihat</a>
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM ncr  WHERE status = 'done'");
            $stmt->execute();
            $result = $stmt->fetchAll();
          ...
          <?php } else { ?>
            <li><a href="listNcrDihantar.php?page=<?php echo $page-1 ?>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
          <?php
          }
          for ($i=1; $i<=$total_pages; $i++)
            if ($i == $page)
              echo "<li class=\"active\"><a href=\"listNcrDihantar.php?page=$i\">$i</a></li>";
            else
              echo "<li><a href=\"listNcrDihantar.php?page=$i\">$i</a></li>";
          ?>
