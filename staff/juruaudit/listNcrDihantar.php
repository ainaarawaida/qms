    ...
    <tr>
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM ncr WHERE staff_no = '$uid' AND  status = 'post' LIMIT $start_from, $per_page");
        $stmt->execute();
      ...
      <tr>
        ...
        <td>
          <a href="kemaskiniNcr.php?no_id=<?php echo $readrow['no_id']; ?>" class="btn btn-success btn-xs" role="button">Lihat</a>
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM ncr  WHERE status = 'post'");
            $stmt->execute();
            $result = $stmt->fetchAll();
