    ...
    <tr>
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM juruaudit LIMIT $start_from, $per_page");
        $stmt->execute();
      ...
      <tr>
        ...
        <td><?php echo $readrow['staff_no']; ?></td>
        <td><?php echo $readrow['staff_email']; ?></td>
        <td><?php echo $readrow['staff_noTel']; ?></td>
        ...
        <td>
          ...
          <a href="lihatAuditor.php?edit=<?php echo $readrow['staff_no']; ?>" class="btn btn-success btn-xs" role="button">Lihat</a>
      ...
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM juruaudit");
            $stmt->execute();
            $result = $stmt->fetchAll();
          ...
          <?php } else { ?>
            <li><a href="listAuditor.php?page=<?php echo $page-1 ?>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
          <?php
          }
          for ($i=1; $i<=$total_pages; $i++)
            if ($i == $page)
              echo "<li class=\"active\"><a href=\"listAuditor.php?page=$i\">$i</a></li>";
            else
              echo "<li><a href=\"listAuditor.php?page=$i\">$i</a></li>";
          ?>
